...
export const tvShowRemoteDataSourceImpl: TVShowRemoteDataSource = {
  async getTVShowByClasification(
    clasification: string,
    page?: number,
  ): Promise<TVShowsResponse> {
  ...
  async getTVShowDetail(tvShowId: string): Promise<TVShowDetailResponse> {
    const resp = await movieDB.get<TVShowDetailResponse>(
      `tv/${tvShowId}?append_to_response=videos,reviews,credits`,
    );
    return resp.data;
  },
